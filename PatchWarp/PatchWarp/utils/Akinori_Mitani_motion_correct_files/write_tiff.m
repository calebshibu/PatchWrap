...
%	end
        ...
        for ii=1:size(stack,3)
            if(numel(info)>=ii && isfield(info,'ImageDescription') && ischar(info(ii).ImageDescription))
                tagstruct.ImageDescription=info(ii).ImageDescription;
            elseif((numel(info)==1) && isfield(info,'ImageDescription')) || (numel(info)>=ii && isfield(info,'ImageDescription') && ~ischar(info(ii).ImageDescription))
