function iglusnfr_patchwarp(input_file_path, base_output_path)
    % Display the provided paths for confirmation
    disp(['Input File Path: ', input_file_path]);
    disp(['Output File Path: ', base_output_path]);

    % Add PatchWarp directory to MATLAB path
    patchwarp_path = '/root/capsule/code/PatchWarp';
    addpath(genpath(patchwarp_path));

    % Set up the operations structure with the provided file paths
    % Extract the last directory name from input path
    parts = strsplit(input_file_path, '/');
    last_dir = parts{end-1};
    
    % Create the corresponding output directory path
    output_file_path = fullfile(base_output_path, last_dir);
    
    % If the last part is empty (i.e., it's a directory), use the parent directory name
    if isempty(last_dir)
    ...
    ops.source_path = input_file_path;
    ops.save_path = output_file_path;
    ...
    % Run PatchWarp with the configured parameters
    patchwarp(ops);
    
    % Define source and destination directories
    sourceDir = fullfile(output_file_path, 'pre_warp');
    destinationDir = output_file_path;
    ...
    for i = 1:length(files)
        ...
        srcFilePath = fullfile(sourceDir, files(i).name);
        destFilePath = fullfile(destinationDir, files(i).name);
        
        % Move file or folder
        if ~files(i).isdir
            movefile(srcFilePath, destFilePath); % Move files
        else
            % If it's a directory, move it recursively
            movefile(srcFilePath, destFilePath, 'f'); % 'f' flag for recursive move
        end
    end
    
    % Remove the source directory after ensuring it's empty
    rmdir(sourceDir, 's'); % Use 's' flag to remove non-empty directories recursively
    
    % Define the directories to be deleted
    dirsToDelete = {'downsampled', 'target'};
    
    % Loop through each directory and delete it
    for i = 1:length(dirsToDelete)
        dirPath = fullfile(output_file_path, dirsToDelete{i});
        
        % Check if the directory exists before attempting to delete
        if isfolder(dirPath)
            rmdir(dirPath, 's'); % 's' flag ensures non-empty directories are removed
...
%% usage: iglusnfr_patchwarp('/root/capsule/data/iGluSnFR_simulations/1/SIMULATION_scan_00001_1_Trial1.tif', '/root/capsule/scratch')